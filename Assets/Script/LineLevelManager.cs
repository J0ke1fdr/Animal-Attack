...
public class LineLevelManager : MonoBehaviour
{
    private int level = 1;
    private int maxCountInMap = 20;              //地图上最多同时存在的怪物数目
    private int enemyRemainCountLevel = 70;           //当前关卡剩余未生成怪物数
    ...
    private int[] AnimalNuminThePoint = { 0, 0, 0, 0, 0, 0, 0 };
    ...
    private void FixedUpdate()
    {
        ...
        {
            if (AnimalNuminThePoint[currentPoint] < 10)
            {
                CreateAnimalATPoint(CreateAnimalPoint[currentPoint]);
                AnimalNuminThePoint[currentPoint]++;
            }
            else
            {
                if (currentPoint < 7)
    ...
    private void CreateAnimalATPoint(Transform Point)
    {
        ...
        {
            AnimalList.Add((GameObject)Instantiate(crab, Point.position, Point.rotation));
            enemyRemainCountLevel--;
        }

        if ((enemyRemainCountLevel % 9) == 0)
        {
            AnimalList.Add((GameObject)Instantiate(pig, Point.position, Point.rotation));
            enemyRemainCountLevel--;
        }
    }
